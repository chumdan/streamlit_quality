...
if 'data' in st.session_state and st.session_state.data is not None:
    ...
    # 배치 ID 컬럼 선택 또는 인덱스 사용
    use_index_as_batch = st.checkbox("데이터 인덱스를 배치 ID로 사용", value=len(potential_batch_cols) == 0)
    
    # 배치 이름으로 사용할 추가 컬럼 선택 (선택 사항)
    use_name_column = st.checkbox("배치 이름 표시에 사용할 추가 컬럼 선택", help="배치 ID 외에 제품명, 생산일자 등 배치를 더 쉽게 식별할 수 있는 컬럼을 선택합니다.")
    ...
    if not use_index_as_batch:
        ...
        if batch_col in data.columns:
            ...
            st.error(f"선택한 '{batch_col}' 컬럼이 데이터에 없습니다.")
            data_analysis = data.copy()
            batch_ids = data_analysis.index.tolist()
    ...
    if selected_vars:
        ...
        if selected_batch:
            ...
            # 그래프 열 수 계산
            n_cols = min(3, len(selected_vars))
            n_rows = math.ceil(len(selected_vars) / n_cols)
            
            # 서브플롯 생성
            fig = make_subplots(
                rows=n_rows, 
                cols=n_cols,
                subplot_titles=[f"{var} (Z-score: {batch_zscores[var]:.2f})" for var in selected_vars]
            )
            ...
            if outlier_vars:
                ...
                {', '.join([f"**{var}** (Z-score: {batch_zscores[var]:.2f})" for var in outlier_vars])}
                ...
                st.markdown(f"""
                #### 가장 큰 특이값을 가진 변수: {worst_var} (Z-score: {batch_zscores[worst_var]:.2f})
                
                이 변수는 평균보다 {abs(batch_zscores[worst_var]):.1f}배 표준편차만큼 {'높습니다' if batch_zscores[worst_var] > 0 else '낮습니다'}.
            ...
            # 상호작용 분석 (옵션)
            if len(outlier_vars) >= 2 and st.checkbox("변수 간 상호작용 분석"):
                st.markdown("### 특이 변수 간 상호작용")
                ...
                # 상관관계 분석
                corr = data_analysis[[var1, var2]].corr().iloc[0, 1]
                st.markdown(f"""
                **상관계수: {corr:.2f}**
                
                {'변수 간에 강한 상관관계가 있습니다.' if abs(corr) > 0.7 else
                 '변수 간에 중간 정도의 상관관계가 있습니다.' if abs(corr) > 0.3 else
                 '변수 간에 약한 상관관계가 있습니다.'}
                
                {'이 두 변수는 공통 원인에 의해 영향을 받았을 가능성이 높습니다.' if abs(corr) > 0.5 else ''}
                """)
    else:
